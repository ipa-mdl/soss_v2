...
template <typename T> T get_json_float(Json::const_reference json_node)
{
  if (json_node.is_number_float()) {
      return json_node.get<T>();
  }else{
      std::string s = json_node.get<std::string>();
...
template<typename T> Json::reference add_json_float(
        const xtypes::DynamicData::ReadableNode& soss_node,
        Json::pointer json_node)
{
  ...
  {
    return add_json_node(soss_node, json_node) = value;
  }
  else
  {
    return add_json_node(soss_node, json_node) = std::to_string(value);
  }
}
